...
| UDT | driver-provided | driver-provided | `transport/udt` is a stable Bootstrap/Dialer adapter boundary. Without an injected Driver it returns a deterministic unsupported error; native UDT support is intentionally outside the core package. |
| RXTX/serial | driver-provided client | driver-provided client | `transport/rxtx` is a serial client transport boundary with normalized baud/data/stop/parity settings. Without an injected Driver it returns a deterministic unsupported error. |
...
- UDT/RXTX core validation covers adapter contracts and fail-fast unsupported semantics; runtime protocol/serial behavior belongs to injected driver packages.
