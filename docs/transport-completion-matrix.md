...
| SCTP | Linux `epoll` and `std` when kernel SCTP is available | unsupported | `transport/sctp` exposes one-to-one stream sockets through `ServerBootstrap`/`Dialer`; non-Linux platforms and completion pollers fail fast with explicit unsupported errors. |
...
| UDT | driver-provided | driver-provided | `transport/udt` is a stable Bootstrap/Dialer adapter boundary, and `transport/udt/driver` provides Backend/function adapter helpers for external implementations. Without an injected Driver it returns a deterministic unsupported error; native UDT support is intentionally outside the core package. |
| RXTX/serial | driver-provided client | driver-provided client | `transport/rxtx` is a serial client transport boundary with normalized baud/data/stop/parity settings, and `transport/rxtx/driver` provides BackendFunc adapter helpers for external implementations. Without an injected Driver it returns a deterministic unsupported error. |
...
| QUIC RFC9000 connection | system UDP socket | system UDP socket | `transport/quic/rfc9000` exposes a complete RFC 9000 QUIC v1 connection adapter backed by TLS 1.3 packet protection, ALPN, 0-RTT opt-in path, bidirectional streams, unidirectional streams, datagrams, WebTransport prerequisites, connection state/stats, qlog writer factory, and provider capability metadata. |
...
- UDT/RXTX validation covers core adapter contracts, typed-nil driver safety, function-based driver helper packages, and fail-fast unsupported semantics; runtime protocol/serial behavior belongs to injected driver packages.
