...
| L2 frame transport | Linux AF_PACKET native driver; macOS/BSD BPF driver boundary; Windows Npcap driver boundary | driver-provided | `transport/l2` exposes `ServerBootstrap`/`Dialer` over injectable `Driver`/`Endpoint` contracts. Linux AF_PACKET is native; `transport/l2/bpf` and `transport/l2/npcap` provide optional platform driver adapters in core without binding cgo or dynamic libraries. |
