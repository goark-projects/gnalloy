...
| QUIC | inherits UDP | inherits UDP | Current package is a QUIC packet engine over UDP. It does not yet implement full TLS 1.3 handshake, loss recovery, congestion control, stream multiplexing, or connection migration. |
...
- QUIC transport validation means packet parsing/routing over UDP unless a future test explicitly covers full QUIC connection semantics.
