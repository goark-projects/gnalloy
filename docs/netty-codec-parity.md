...
| `JdkZlibEncoder/JdkZlibDecoder`、brotli/snappy/lz4/zstd/bzip2/lzma compression codecs | `codec/compression`、`codec/compression/brotli`、`codec/compression/snappy`、`codec/compression/lz4`、`codec/compression/zstd`、`codec/compression/bzip2`、`codec/compression/lzma` | done | 根包支持标准库 gzip/zlib；扩展子包支持 Brotli、Snappy stream、LZ4 stream、Zstandard、BZip2 和 classic LZMA，解码器提供最大解压大小限制。 |
...
| Netty 模块 | 状态 | 原因 |
| --- | --- | --- |
| compression (`brotli`, `snappy`, `lz4`, `zstd`, `bzip2`, `lzma`) | done | 已拆到独立 `codec/compression/*` 子包，避免根包直接绑定外部算法依赖。 |
