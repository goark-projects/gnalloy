...
| QUIC RFC9000 connection stack | `transport/quic`、`transport/quic/rfc9000`、`transport/quic/provider`、`transport/quic/provider/quicgo` | done | `transport/quic` 是 Gnalloy 门面，生产协议栈全部委托 quic-go 承接 RFC 9000 QUIC v1、TLS 1.3 packet protection、ALPN、双向 stream、单向 stream、datagram、provider capability inspection、localhost 互通和显式启用的外部互通测试；不保留自研 packet/frame/runtime 栈。 |
