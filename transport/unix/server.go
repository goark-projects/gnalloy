...
// Transport 把 ServerBootstrap 接到 Unix domain socket 生命周期。
...
func (t *Transport) Bind(ctx context.Context, cfg bootstrap.ServerConfig) (bootstrap.Server, error) {
	...
	listener, err := listenUnix(cfg.Address, listenOptions)
	if err != nil {
		return nil, err
	}
	...
	if childOptions.iouringFixed {
		for _, worker := range cfg.WorkerGroup.Loops() {
			if _, err := server.allocatorFor(worker); err != nil {
				_ = closeFD(listener.fd)
				_ = cleanupSocket(listener.addr)
	...
	server.acceptor = &acceptor{id: t.nextChannelID(), server: server, fd: listener.fd}
	bossLoop, err := cfg.BossGroup.RegisterNext(ctx, server.acceptor, transport.ReadyRead, func(loop *transport.EventLoop, handler transport.EventHandler) error {
		a := handler.(*acceptor)
		a.loop = loop
...
// Server 表示 Unix domain socket 服务端句柄。
type Server struct {
	addr Address
...
func (s *Server) Close() error {
	s.once.Do(func() {
		...
		if s.acceptor != nil {
			...
			} else if err := s.acceptor.Close(); err != nil && first == nil {
				first = err
			}
		}
		...
		if err := cleanupSocket(s.addr); err != nil && first == nil {
			first = err
		}
		s.err = first
	})
