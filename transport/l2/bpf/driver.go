...
// Backend 是真实 BPF 驱动需要实现的最小打开接口。
type Backend interface {
	// OpenBPF 使用规范化配置打开一个 BPF Endpoint。
	OpenBPF(ctx context.Context, cfg Config) (l2.Endpoint, error)
}

// BackendFunc 允许普通函数作为 BPF Backend 使用。
type BackendFunc func(ctx context.Context, cfg Config) (l2.Endpoint, error)

// OpenBPF 实现 Backend。
func (f BackendFunc) OpenBPF(ctx context.Context, cfg Config) (l2.Endpoint, error) {
	return f(ctx, cfg)
}

// Driver 把独立 BPF 后端适配成 transport/l2.Driver。
type Driver struct {
	Backend Backend
	Config  Config
}

// NewDriver 创建 BPF Driver 适配器。
