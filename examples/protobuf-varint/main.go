...
import (
	...
	"goark.dev/gnalloy/codec/protobuf"
...
func main() {
	...
	opts := exampleconfig.Register(fs, ":9012")
	...
	server, err := bootstrap.NewServerBootstrap().
		...
		ChildInitializer(func(ch channel.Channel) error {
			decoder, err := protobuf.NewVarint32FrameDecoder(1 << 20)
			if err != nil {
				return err
			}
			if err := ch.Pipeline().AddLast("prepender", protobuf.NewVarint32LengthFieldPrepender()); err != nil {
				return err
			}
			if err := ch.Pipeline().AddLast("frame", decoder); err != nil {
				return err
			}
			return ch.Pipeline().AddLast("echo", protobufEchoHandler{})
		}).
	...
	fmt.Printf("gnalloy protobuf-varint echo listening on %s backend=%s boss=%d workers=%d reuseport=%v mmap=%v\n",
...
type protobufEchoHandler struct{}

func (protobufEchoHandler) ChannelRead(ctx *channel.HandlerContext, msg any) {
...
func (protobufEchoHandler) ExceptionCaught(ctx *channel.HandlerContext, _ error) {
